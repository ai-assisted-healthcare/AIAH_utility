...
class BasicViewer:
    ...
    Args:
        ...
        start_view: plot image in axial, sagittal or coronal view; expects: (0,1,2)
        ...
        laplace: outline edges of label mask
    Returns:
    ...
    def _plot_slice(self, im_slice: int, with_mask: bool, px_range: Tuple[int, int], view: int) -> None:
        """Plot slice of image"""
        if im_slice > self.x.shape[view]:
            print("Out of image borders")
            return
        fig, ax = plt.subplots(1, 1, figsize=self.figsize)
        get_slice = lambda x, view, pos: (x[pos, :, :] if view == 0 else x[:, pos, :] if view == 1 else x[:, :, pos])
        ...
        if with_mask and isinstance(self.y, (torch.Tensor, np.ndarray)):
            ...
            if self.laplace:
                ...
                cmap = mpl.cm.jet(np.linspace(0, 1, int(self.y.max())))
                cmap -= 0.4
                cmap = cmap.clip(0, 1)
                cmap = mpl.colors.ListedColormap(cmap)

                alpha = np.zeros(edge_slice.shape)
                alpha[edge_slice > self.background_threshold] = 0.9
